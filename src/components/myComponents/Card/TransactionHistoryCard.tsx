...
interface HistoryCardProps {
  ...
  index: number; // Use index as a temporary identifier
  onDelete: (index: number) => void; // Callback to handle delete
}
...
function TransactionHistoryCard({ type, category, amount, description, date, index, onDelete }: HistoryCardProps) {
      ...
      <div>
        <Card className="p-4">
          ...
          <div className="flex justify-end">
            <Button className="bg-red-500 text-white" onClick={() => onDelete(index)}>Delete</Button>
          </div>
        </Card>
      </div>
    </>
